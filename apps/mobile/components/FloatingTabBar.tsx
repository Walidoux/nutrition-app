...
export default function FloatingTabBar({ state, navigation }: BottomTabBarProps) {
  ...
  }, [activeIndex, bounce, state.index])
  ...
  return (
    <View pointerEvents='box-none' className='absolute left-4 right-4' style={{ bottom: Math.max(insets.bottom, 12) }}>
      <BlurView
        intensity={80}
        tint={tint}
        style={[
          styles.surface,
          scheme === 'dark' ? { backgroundColor: 'rgba(22,22,26,0.55)' } : { backgroundColor: 'rgba(255,255,255,0.75)' }
        ]}
        onLayout={(e) => {
            ...
            return (
              <Pressable
                ...
                <Animated.View style={[styles.iconBox, animatedIconStyle]}>
                  <Ionicons name={iconName} size={ICON_SIZE} color={color} style={styles.iconGlyph} />
                </Animated.View>
              </Pressable>
            )
          })}
